...
This repo is split by concern. `docs/` holds architecture and product notes, `frontend/` is the active UI workspace, and `backend/` and `rag-backend/` are future service boundaries. Use `infra/` for deployment notes and `scripts/` for helper automation. Keep browser-generated artifacts out of source control, especially `.next/`, `.playwright-mcp/`, and `*.tsbuildinfo`.
...
- `npm --prefix frontend run build` builds production output.
- `npm --prefix frontend run check` runs Prettier check and ESLint.
- `npm --prefix frontend run format` rewrites files with Prettier.
...
There is no dedicated unit test suite yet. Treat `npm --prefix frontend run check` and `npm --prefix frontend run build` as the baseline verification steps before you finish work. For UI changes, validate the affected route in the browser, especially `/` and `/dashboard`.
...
Follow the service-level guide in `frontend/AGENTS.md` when editing that app. If you add new generated files, update `.gitignore` instead of committing build artifacts.
